...
describe('mainScreenHelpers', () => {
  it('converts approval and user-input fixtures from the contract manifest', () => {
    ...
    const approval = helpers.toPendingApproval(manifest.fixtures.notification.params);
    const userInput = helpers.toPendingUserInputRequest(manifest.fixtures.pendingUserInput);
  ...
  it('exposes only executable slash commands for the active agent and chat state', () => {
    const compact = helpers.findSlashCommandDefinition('compact');
    const goal = helpers.findSlashCommandDefinition('goal');
    const permissions = helpers.findSlashCommandDefinition('permissions');
    expect(compact).toBeNull();
    expect(goal && helpers.isSlashCommandAvailable(goal, availability)).toBe(false);
    expect(permissions && helpers.isSlashCommandAvailable(permissions, availability)).toBe(false);
  });

  it('keeps transient revalidation failures behind a hydrated chat snapshot', () => {
    expect(helpers.shouldSurfaceChatLoadError(true, 'thread-1', 'thread-1', 3)).toBe(false);
    expect(helpers.shouldSurfaceChatLoadError(false, 'thread-1', 'thread-1', 3)).toBe(true);
    expect(helpers.shouldSurfaceChatLoadError(true, 'thread-2', 'thread-1', 3)).toBe(true);
  ...
  it('keeps successful git clone responses quiet', () => {
    expect(
      helpers.formatGitCloneFailureMessage({
        code: 0,
        stdout: '',
        stderr: '',
        cloned: true,
  ...
  it('hydrates persisted bridge UI surfaces by thread', () => {
    const result = helpers.parseChatBridgeUiSurfaces(
      JSON.stringify({
        version: 1,
        entries: {
          'thread-1': [
  ...
  it('builds an optimistic replacement surface for new goal slash commands', () => {
    expect(helpers.parseGoalSlashObjective('/goal Now verify the PR')).toBe('Now verify the PR');
    expect(helpers.parseGoalSlashObjective('/goal')).toBeNull();
    expect(helpers.parseGoalSlashObjective('/model gpt-5.5')).toBeNull();
