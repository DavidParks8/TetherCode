...
async function renderTerms(url: string | null, onOpenDrawer = jest.fn()): Promise<ReactTestRenderer> {
  ...
  if (!tree) throw new Error('Expected terms screen tree');
  return tree;
}
...
describe('TermsScreen behavior', () => {
  ...
  it('alerts for unsupported links and open failures', async () => {
    ...
    expect(Alert.alert).toHaveBeenCalledWith('Could not open link', 'Please open the terms URL manually.');
