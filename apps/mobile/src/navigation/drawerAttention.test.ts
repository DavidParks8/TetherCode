...
describe('buildDrawerAttentionModel', () => {
  ...
  it('disambiguates duplicate folder basenames for native pickers and selection', () => {
    const model = buildDrawerAttentionModel({
      chats: [
        chat('first', { cwd: '/repo-a/mobile', updatedAt: '2026-07-20T00:29:00.000Z' }),
        chat('second', { cwd: '/repo-b/mobile', updatedAt: '2026-07-20T00:28:00.000Z' }),
      ],
      agents,
      runIndicatorsByThread: {},
      pendingApprovals: [],
      pendingUserInputs: [],
      selectedFolderKey: '/repo-a/mobile',
      workspaceChatLimit: null,
    });

    expect(getDrawerFolderPickerLabels(model.folderOptions)).toEqual([
      'All folders',
      'mobile — /repo-a/mobile',
      'mobile — /repo-b/mobile',
    ]);
    expect(model.selectedFolderLabel).toBe('mobile — /repo-a/mobile');
  });
