...
TETHERCODE_BRIDGE_FORCE_SOURCE_BUILD="${TETHERCODE_BRIDGE_FORCE_SOURCE_BUILD:-}"
...
BRIDGE_PREVIEW_HOST="$BRIDGE_HOST"
...
EXISTING_TOKEN=""
EXISTING_FORCE_SOURCE_BUILD=""
if [[ -f "$SECURE_ENV_FILE" ]]; then
  EXISTING_TOKEN="$(awk -F= '/^BRIDGE_AUTH_TOKEN=/{print substr($0, index($0, "=")+1)}' "$SECURE_ENV_FILE" | head -n1)"
  EXISTING_FORCE_SOURCE_BUILD="$(awk -F= '/^TETHERCODE_BRIDGE_FORCE_SOURCE_BUILD=/{print substr($0, index($0, "=")+1)}' "$SECURE_ENV_FILE" | head -n1)"
fi
...
TETHERCODE_BRIDGE_FORCE_SOURCE_BUILD="${TETHERCODE_BRIDGE_FORCE_SOURCE_BUILD:-${EXISTING_FORCE_SOURCE_BUILD:-false}}"
if [[ "$TETHERCODE_BRIDGE_FORCE_SOURCE_BUILD" != "true" ]] && [[ "$TETHERCODE_BRIDGE_FORCE_SOURCE_BUILD" != "false" ]]; then
  echo "error: TETHERCODE_BRIDGE_FORCE_SOURCE_BUILD must be 'true' or 'false'." >&2
  exit 1
fi
...
ACP_AGENT_MANIFEST="$ROOT_DIR/.tethercode/agents.json"
ACP_AGENT_ROOTS="$ROOT_DIR/.tethercode/agents"
...
BRIDGE_PREVIEW_HOST=$BRIDGE_PREVIEW_HOST
