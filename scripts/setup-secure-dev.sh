...
if [[ -f "$SECURE_ENV_FILE" ]]; then
  ...
  EXISTING_FORCE_SOURCE_BUILD="$(awk -F= '/^CLAWDEX_BRIDGE_FORCE_SOURCE_BUILD=/{print substr($0, index($0, "=")+1)}' "$SECURE_ENV_FILE" | head -n1)"
fi
...
CLAWDEX_BRIDGE_FORCE_SOURCE_BUILD="${CLAWDEX_BRIDGE_FORCE_SOURCE_BUILD:-${EXISTING_FORCE_SOURCE_BUILD:-false}}"
if [[ "$CLAWDEX_BRIDGE_FORCE_SOURCE_BUILD" != "true" ]] && [[ "$CLAWDEX_BRIDGE_FORCE_SOURCE_BUILD" != "false" ]]; then
  echo "error: CLAWDEX_BRIDGE_FORCE_SOURCE_BUILD must be 'true' or 'false'." >&2
  exit 1
fi
