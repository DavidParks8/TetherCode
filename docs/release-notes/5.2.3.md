...
- Notification payloads are bounded but not content-free. Expo receives the push
  token, visible title/body, event type, notification/profile/registration
  identities, thread id, approval id when applicable, and a short reply preview
  on completion when available. Generic text contains the bridge project name.
...
- Push delivery requires outbound HTTPS from the bridge but does not require
  inbound public bridge access.
...
- Enabling notifications transmits a device push token, notification text and
  routing identifiers, and possibly a short reply-text preview through Expo and
  the platform push services for delivery. This is
