# Code of Conduct

## Our Pledge

We want `tethercode` to be a respectful, technically serious, and inclusive project.
Contributors, maintainers, and community members are expected to engage professionally and constructively.

## Expected Behavior

- Be respectful in language, tone, and feedback.
- Assume good intent, but be clear and direct when discussing technical issues.
- Focus on facts, reproducibility, and maintainable solutions.
- Accept feedback gracefully and collaborate in good faith.
- Help keep discussions relevant to the project.

## Unacceptable Behavior

- Harassment, intimidation, discrimination, or personal attacks.
- Sexualized language or unwelcome sexual attention.
- Doxxing, threats, or sustained disruption of discussions.
- Bad-faith trolling, repeated derailment, or hostile review behavior.
- Publishing private or sensitive information without permission.

## Scope

This Code of Conduct applies to:

- GitHub issues, pull requests, discussions, and reviews
- project chat spaces and community channels
- public or private interactions where someone is representing this project

## Enforcement

Project maintainers may remove, edit, or reject comments, commits, issues, pull requests, and other contributions that violate this Code of Conduct.
They may also temporarily or permanently restrict participation when necessary.

## Reporting

If you experience or witness unacceptable behavior, report it privately through GitHub's reporting
and repository moderation tools.

Reports will be reviewed as promptly and discreetly as possible.

## Attribution

This document is adapted from the Contributor Covenant, version 2.1:
<https://www.contributor-covenant.org/version/2/1/code_of_conduct/>
